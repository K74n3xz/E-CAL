        ...
        val alarm = event.alarms[0]
            ...
            .let {
                if (interval == null && repeat == null) {
                    ...
                    it.withProperty(net.fortuna.ical4j.model.property.Duration(interval))
                        .withProperty(Repeat(repeat))
                } else {
                    throw IllegalArgumentException("`interval` and `repeat` must be assigned values simultaneously or neither must be assigned a value. (May the instance of Alarm be broken?)")
                }
            }
